...
- `script.js`: navigation, atmosphere transforms and continuous cape animation. The hero's body stays fixed. On every screen size, the adventurer is 18% larger with boots anchored to the ground, the cape has 3.2× the cloth displacement and a 45% quicker wind cycle, and clouds/mist travel more actively. All viewports share the same wind settings and continuous phase. Clouds and mist start independently of character loading; only one atlas frame is processed.
...
- `intro.js` / `intro.css`: a gateway entrance tied to artwork readiness. An illuminated arch opens onto the landscape as the centered logo fades out; the header logo fades in at its final position and the typography rises into place. The logo starts only after decoding, and the sequence counts visible time so background tabs cannot skip it. Resizing does not interrupt the entrance. Skip intro or Escape opens the page immediately; reduced-motion settings omit the animated reveal. Asset deadlines and a visible-time bootstrap fallback prevent stalled requests or module failures from trapping visitors.
